...
// Region centroids for the client-side "what's on near me" matcher.
export async function fetchRegionIndex() {
  const regions = await fetchEventsByRegion();
  return regions.map((r) => {
    const pts = r.events.filter((e) => e.lat != null && e.lng != null);
    const lat = pts.length ? pts.reduce((s, e) => s + e.lat, 0) / pts.length : null;
    const lng = pts.length ? pts.reduce((s, e) => s + e.lng, 0) / pts.length : null;
    return { slug: r.slug, name: r.name, count: r.events.length, lat, lng };
  });
}
