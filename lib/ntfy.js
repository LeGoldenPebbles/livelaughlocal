 ...
 *
 * Config:
 *  - NTFY_TOPIC_LLL  overrides the topic. Set it to the Spaces Please chat
 *                    topic if you would rather these land on an existing
 *                    subscription; set it to '' to disable pushes entirely.
 *  - NTFY_SERVER     custom/self-hosted ntfy server (defaults to ntfy.sh).
 *
 * The default topic is unguessable so pushes work without env setup, but anyone
 * holding the string can read the feed - treat it like a shared secret and
 * rotate via the env var if it leaks.
 *
 ...
 */
const DEFAULT_TOPIC = 'livelaughlocal-review-8c4f1e7b2a';
...
export async function sendSubmissionAlert({ featured = false, hasImage = false } = {}) {
  ...
  return sendAlert({
    title: featured
      ? 'New FEATURED story submitted (GBP 100)'
...
export async function sendSubmissionConfirmedAlert() {
  return sendAlert({
    title: 'Submitted story confirmed by email',
